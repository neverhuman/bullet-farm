...
pub(crate) const UNGATED: &[UngatedRow] = &[
    ...
    UngatedRow {
        ...
        evidence: "design only — `docs/architecture/evolutionary-control.md` (post-V1 section) and policy `route_policy.evolutionary_authority=false`; no recipe, archive, or promotion code path",
        owner: Owner::Local("post-V1 engineering after G2; never by flipping the policy bit first"),
        next: none("do not start a campaign; `docs/phase-9-10.md` is post-V1"),
        blocking: "no for V1 — post-V1 by the frozen contract; it blocks only an evolutionary product claim, not `release.transaction-demo`",
    },
    UngatedRow {
        gap_id: "G13",
        claim: "Seven of fifteen Portal spec surfaces (Cognitive Router, Fusion Lab, Context Lineage, Quota/Capacity, Struggle, Behavior, Workspace Hygiene) have no durable ledger subject and render explicit UNKNOWN, and the Portal is not packaged or embedded.",
    ...
    UngatedRow {
        gap_id: "G14",
        claim: "farmd's public surface is the authenticated command/snapshot/SSE subset plus five read-only projections, not the designed control plane; no signed dispatch settles a public command beyond PENDING, UNKNOWN, or FAILED.",
