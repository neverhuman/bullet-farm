...
pub(crate) const UNGATED: &[UngatedRow] = &[
    ...
    UngatedRow {
        ...
        blocking: "no for V1 — self-tuning optimization is post-V1; `linux-preview` surfaces an extra evolution diagnostic that cannot alter the `universal-v1` dependency closure",
    },
    UngatedRow {
        gap_id: "G13",
        claim: "Six of fifteen Portal spec surfaces (Cognitive Router, Fusion Lab, Quota/Capacity, Struggle, Behavior, Workspace Hygiene) have no durable ledger subject and render explicit UNKNOWN; same-origin Rust embedding and packaged-farmd browser tests exist only as component evidence, with no signed distribution or install receipt.",
        why: "A projection without a ledger subject cannot be truthful, and an embedded component build is not a released product until signed archive and installer receipts are independently admitted.",
        acceptance: "After G2/G3, add the missing ledger subjects, project all fifteen surfaces with watermark-bound truth, and ship the embedded Portal through the signed archive and install lifecycle gates.",
        class: GateClass::Transaction,
        evidence: "COMPONENT only — nine projected surfaces, CSRF/202, PENDING→UNKNOWN, SSE STALE, exact Portal bundle verification, same-origin embedding, and packaged-farmd browser proof; six surfaces stay UNKNOWN and no signed archive/install receipt exists",
        owner: Owner::Local("Portal + farmd owners after G2/G3 (V1-S5)"),
        next: none(
            "packaged-farmd tests are component evidence; require the missing ledger subjects plus signed package and install receipts",
        ),
