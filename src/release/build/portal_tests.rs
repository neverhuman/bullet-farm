...
fn portal_manifest_refuses_ambiguous_or_unsafe_projections() {
    ...
    let unsafe_size = canonical.replacen("\"total_size\":0", "\"total_size\":9007199254740992", 1);
    ...
    let duplicate = canonical.replacen("\"total_size\":0", "\"total_size\":0,\"total_size\":0", 1);
    ...
    let mut pretty = serde_json::to_vec_pretty(&rooted_manifest(&value)).expect("pretty manifest");
    pretty.push(b'\n');
    assert!(
    ...
    hostile_inventories.push(hostile);
    let mut hostile = value.clone();
    hostile["files"] = serde_json::json!([file("assets/app.js", "text/javascript; charset=utf-8")]);
    hostile_inventories.push(hostile);
    let mut hostile = value.clone();
    hostile["files"] = serde_json::json!([
    ...
    assert!(
        read_regular_bounded(&oversized_lock, MAX_PACKAGE_LOCK_BYTES, "package-lock.json").is_err()
    );
