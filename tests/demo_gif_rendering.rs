...
const FIXTURE: &str = r#"
import hashlib,json,os,shutil,signal,struct,subprocess,sys,time,zlib
...
def portal(name,colorful=False):
 ...
 for frame in range(2):
  ...
  frames.append({'file':file,'elapsed_ms':100.123+frame*123.456,'sha256':sha(path/file),
...
if case=='pixels':
 ...
 subprocess.run([str(ffmpeg),'-nostdin','-v','error','-y','-f','rawvideo','-pixel_format','rgb24',
  '-video_size','16x32','-framerate','1','-i',str(root/'wrong.rgb'),'-c:v','ffv1','-pix_fmt','bgr0',
  '-f','nut',str(wrong/'master.nut')],check=True,timeout=10)
 ...
 observed=subprocess.run([str(ffmpeg),'-nostdin','-v','error','-i',str(wrong/'master.nut'),
  '-fps_mode','passthrough','-c:v','rawvideo','-pix_fmt','rgb24','-f','framehash','-hash','sha256','-'],
 ...
 changed['fidelity']['master_decoded_timing']=[{'pts':int(row[2]),'duration':int(row[3]),'timebase':timebase} for row in frames]
