...
The [11 September audit](deep-audit-20260911.md) preserves its159-file baseline;
...
| H1 / Kernel | Restored authentication/alias/early paint/exact Submissions and containment refusal. Current bb405556 integrates session-bound CLI reads and repaired UI fixtures;16 focused+11 Rust UI pass independent review. | Durable Head, owner-epoch callbacks/mutation acknowledgements, real WorkPackage/Candidate producer and contained native lifecycle. |
| H2 / Kernel | Exact68ffa9f7 required-r6 passed1270 fast+5 lint-receipt+34 contract and all five lanes. Current bb405556 actual inventory is1324 total/1278 standalone; required-r7 failed:11 passed, two mission request-count assertions failed,1265 unexecuted. | Complete current required/Tuiwright and continuous source proof; prior raw-report loss, r4 docs failure and r5 three40s timeouts remain retained. |
| H2 / Portal | Exacte178db3 full required7e526 accepted360 fast/build and five lanes. Currentb291 coverage passes372 exact tests, unchanged global/seven-file floors and continuous monitor/final read-back; changed/restored compile negative remains accepted. | Current full required, browser campaigns, hosted admission and native custody remain separate; prelaunch a259 identity refusal had zero executed tests and is preserved. |
| U1 / TUI | Earlier canonical operator-tui12/12 retained126 artifacts and independent review. Current Rust UI6+startup5 pass; authenticated Tuiwright fixture repair is reviewed. | Execute current canonical12-case Tuiwright; installed/provider/continuous source/performance/media acceptance remains open. |
| C1 / delivery | BulletGit9510fd99 audit plumbing77 components accepted; actual pinned native audit fails72/85 with three hard findings and its failed artifact is refused. Nine community templates locally integrated; public guidance remains stale. | Rust evidence-helper migration, actual hosted audit/classification repair, all event/cell artifacts, native custody, aggregate publication/read-back and five protection packets. |
...
Archives r4–r10 retain exact closed source and evidence; [audit continuation](deep-audit-20260911.md#first-measured-implementation-cycle) records hashes and scope. Never discard newer bytes or replay under a new identity.
...
that history; re-read current consumers before each packet. Concurrent Cursor
Kernel task-admission edits remain separately owned/reviewed input and cannot
inherit the earlier899f878 proof or be credited before new acceptance.
...
Keep `POST /api/v1/commands` as the only mutation ingress. Generate Rust, JSON
...
| Candidate / T1,T5,T6 | Generate matching 128-operation, aggregate/per-write UTF-8, gate-count, metadata/preimage and worst-case escaped transport bounds. Fix current Kernel/BulletGit aggregate/gate/transport disagreements. After confirmed termination atomically preserve Candidate/artifacts, terminate Attempt, settle lease/reservation, audit and enqueue verifier. Cleanup requires its preservation receipt. Independent workcells/keys/nonces/UIDs follow ADR0016–0018; prepare the two-approver carrier before signer activation. |
...
qualify cron, and PR execution does not qualify merge-group. No floor reduction,
removed audit, broad exclusion, neutral78 or existing-receipt shortcut is closure.
...
explicitly superseded/refused rows and retired WP-18 excluded as already recorded.
...
Delivery gates are **engineering foundation**, **admitted internal dogfood**, **public preview** and **complete documented product**. Each requires every applicable predicate. Missing/skipped/unavailable/stale results are nonpassing; the twelve-task campaign and seven-day observation follow the first admitted task.
