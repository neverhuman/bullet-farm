...
| G3 | Production Kernel write path | The signed UDS lease RPC/client now component-binds each registered Runner ID/epoch to its connected UID with `SO_PEERCRED`, pins farmd UID plus socket GID/device/inode across connect, and persists server grant/nonce state. It remains unadmitted because peer registration is debug-fixture-only, the farmd signing key is ephemeral, client recovery/read-back metadata is process-local, and product Runner does not construct the client; durable mutation reservation, production JSON-RPC/provider/effect dispatch, CAS/GC, and production restore also remain | DB-clock leases; authenticated ingress; UNKNOWN/FAILED worker; peer- and socket-bound signed UDS component; launch-grant + Linux egress components | Kernel V1-S2/S4; promote the UDS path with operator-admitted durable registry/key custody, durable client recovery, and product Runner wiring; do not remount unauthenticated `HttpLeaseClient` | [`release.md`](../release.md), [ADR 0011](../decisions/0011-signed-launch-grant-and-egress-isolation.md) |
