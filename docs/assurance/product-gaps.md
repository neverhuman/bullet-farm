...
| --- | --- | --- | --- | --- | --- |
| G1 | Hub-only signed install | Checked-in lock is schema 2 and is refused on purpose. Same-UID pathname swap between verify and Cargo/Node/npm exec remains until a signed prebuilt exists | Descriptor-relative setup, no-replace publish, two-run fixture | Operator publishes schema-3 lock + signed prebuilt `bullet-family` | [`release.md`](../release.md), [`runbooks/source-setup.md`](../runbooks/source-setup.md) |
...
| G8 | Security release floor | Hub Jankurai 58 (raw 58), 10 caps, 26 hard findings; no portable CI artifact | Pinned local scan that fails closed | Hard findings to zero; score ≥90; checksum-pinned CI binary | [`release.md`](../release.md) |
...
| G12 | Family `check release` | 26/26 gates `BLOCKED` | Read-only negative inventory; no skip-green nightly | Receipts for G1–G10 | `bullet-family check release --json` |
...
| What is the same-UID install hole? | Between verify and Cargo/Node/npm exec, a same-UID pathname swap can still race. G1. A signed prebuilt is the closer. |
