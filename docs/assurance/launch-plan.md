...
1. **R-07 / RUNNER-FARMD-LEASE-ROUTE-AUDIT** — harden and admit the existing signed UDS RPC/client: peer-credential and descriptor-bound identity, durable nonce/recovery state, persistent `last_acquire`, `advance_attempt_with_authority`, full-subject release check in one SQLite transaction, and product Runner wiring through the bounded client.
