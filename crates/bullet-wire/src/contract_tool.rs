...
fn write_outputs(root: &Path, outputs: &BTreeMap<PathBuf, Vec<u8>>) -> Result<(), WireError> {
    for (relative, bytes) in outputs {
        ...
        let temporary = destination.with_extension("bullet-contract.tmp");
        let mut file = File::create(&temporary).map_err(io_error("create output", relative))?;
        file.write_all(bytes)
            .map_err(io_error("write output", relative))?;
        file.sync_all().map_err(io_error("sync output", relative))?;
        fs::rename(&temporary, &destination).map_err(io_error("replace output", relative))?;
    }
    Ok(())
}
