...
live_inventory() {
  ...
  for name in "$@"; do
    [[ -f "$root/$name" && ! -L "$root/$name" &&
      ...
      "$(stat -c '%s' -- "$root/$name")" -le 65536 ]] || live_die UNSAFE_ARTIFACT
  done
}
...
live_artifacts() {
  ...
  jq -e '.artifact_hashes | type == "array" and length > 0 and all(.[];
    ...
    (.bytes | type == "number" and . >= 0 and . <= 65536 and floor == .))' \
...
live_text() {
  ...
  for file in stdout.txt stderr.txt; do
  ...
  LC_ALL=C grep -aEiq \
    ...
    "$LIVE_TMP/capture-strings" "$root/stdout.txt" "$root/stderr.txt" || code=$?
...
live_capture() {
  ...
  jq -e '
    ...
    .command.argv[0] == ({"claude-session":"claude","codex-session":"codex",
      "cursor-session":"cursor-agent"}[.demo_id])
  ' "$root/capture.json" >/dev/null || live_die UNSUPPORTED_OR_INCOMPLETE_CAPTURE
  live_artifacts "$root" "$root/capture.json" stderr.txt stdout.txt
  live_text "$root"
}
...
live_manifest() {
  ...
  artifacts="$(live_files "$root" capture.json observation.json stderr.txt stdout.txt transcript.txt)"
...
live_stage() {
  ...
  live_artifacts "$root" "$root/manifest.json" capture.json observation.json stderr.txt stdout.txt transcript.txt
