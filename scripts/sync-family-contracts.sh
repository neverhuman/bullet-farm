...
sync_file \
  ...
  "$FAMILY/bullet-kernel/generated/schema_bundle.rs"
sync_file \
...
sync_formal_trace() {
  local source="$1"
  local destination="$2"
  local source_rel="${source#"$HUB/"}"
  local expected
  expected="$(printf '%s\n%s\n%s\n%s' \
    "// Generated by: bash ../bullet-farm/scripts/sync-family-contracts.sh write" \
    "// Source: bullet-farm:${source_rel}" \
    "// DO NOT EDIT BY HAND." \
    "$(cat -- "$source")")"
  if [[ "$MODE" == "check" ]]; then
    if [[ "$(cat -- "$destination")" != "$expected" ]]; then
      echo "generated contract drift: $destination" >&2
      exit 1
    fi
    return
  ...
  mkdir -p -- "$(dirname "$destination")"
  CONTRACT_SYNC_STAGING="$(mktemp "${destination}.XXXXXX")"
  printf '%s' "$expected" >"$CONTRACT_SYNC_STAGING"
  mv -- "$CONTRACT_SYNC_STAGING" "$destination"
  CONTRACT_SYNC_STAGING=""
}
...
for trace in effect-check-ambiguity effect-third-party lease-fence-reclaim; do
  sync_formal_trace \
    ...
    "$FAMILY/bullet-kernel/generated/formal/${trace}.json"
done
